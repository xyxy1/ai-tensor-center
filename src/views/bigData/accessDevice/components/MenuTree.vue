<template>
  <el-col :span="6" class="server-menu">
    <div class="title">
      <el-input size="mini" placeholder="输入关键字进行过滤" v-model="filterText" clearable></el-input>
    </div>
    <ul v-if="menuData" class="menu">
      <li class="menu-item" v-if="item.communityName.includes(filterText)" v-for="(item,index) in menuData" :key="item.communityCode" :class="{sel: currentRow.communityCode == item.communityCode}" @click="onUpdataMenu(item,index)">
        <div class="box">
          <svg-icon icon-class="server" />
          <span>{{item.communityName}}</span>
          <div class="total">{{item.accessCount}}</div>
        </div>
      </li>
    </ul>
  </el-col>
