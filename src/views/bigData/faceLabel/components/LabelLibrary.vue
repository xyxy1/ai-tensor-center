<template>
  <el-col :span="5" class="label-library">
    ...
    <el-tree ref="menuTree" class="menu" node-key="id" :data="menuData">
      <div class="menu-item" slot-scope="{ node, data }" :class="{sel: currentRow.communityCode+''+currentRow.num == data.communityCode+''+data.num, children:!data.children}" @click="onUpdataMenu(data)">
        <el-dropdown @command="onCommand($event, data)" class="more" placement="bottom-start" v-if="!data.appFlag && !data.communityId">
          <div class="icon">
        ...
        <svg-icon icon-class="groupFill" v-if="!data.code" />
        <span>{{data.communityId?data.communityName:data.name}}</span>
...
import { getLabelLibraryList } from "@/api/bigData/faceLabel/index";
...
export default {
  ...
  methods: {
    ...
    onCommand(command, data) {
      if (command == "del") {
        this.$confirm(`确定删除人脸库【${data.name}】？`, "提示", {
          cancelButtonText: "取消",
